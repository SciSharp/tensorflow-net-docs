...
namespace TransferLearningWithInceptionV3
{
    class Program
    {
        static void Main(string[] args)
        {
            TransferLearningWithInceptionV3 inception = new TransferLearningWithInceptionV3();
            inception.Run();
        }
    }

    public class TransferLearningWithInceptionV3
    {
            ...
            using (var sess = tf.Session(graph))
            {
                ...
                for (int i = 0; i < how_many_training_steps; i++)
                {
                    ...
                    var results = sess.run(
                          new ITensorOrOperation[] { train_step },
                    ...
                    if ((i % eval_step_interval) == 0 || is_last_step)
                    {
                        ...
                        float validation_accuracy = sess.run(evaluation_step,
        ...
        public void Predict()
        {
            ...
            // predict image
            var img_path = Path.Join(data_dir, "test_image.jpg");
